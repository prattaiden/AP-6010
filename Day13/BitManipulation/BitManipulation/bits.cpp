...
//function for checking if the binary number is negative
bool IsNegative( int input )
{
    //set int a to a negative number
    //the remaining digits in binary after this negative number are 0 : 10000...00
    //input is then compared with & to a
    //where there are no 1s, it is equated to zero
    //if a 1 exists in the msb position, and is negative
    int a = 0x80000000;
    if((input & a) == a){
        return true;
    }
...
//function to check if bit 1 is present in the input
int NumBitsSet( uint32_t input )
{
    //bit counter to count the times bit 1 appears
    int bitCounter = 0;
    //set a variable uint32 of t to equal a binary number that most right position is 1 LSB
    uint32_t t = 0x00000001;
    //loops through 32 times
    //then shifts input right by one to compare the next digit
    for (int i = 0; i <= 32; i++){
        if ((input & t) == t){
            bitCounter++;
        }
...
//function to get a desired Bytes count from a uint32 input from the user
unsigned char GetByte( uint32_t input, int b )
{
    //assignining uints with values that fill each byte with 1s
    //this is used when we & these uints and the given input
    uint32_t q = 0xFF;
    uint32_t w = 0xFF00;
    uint32_t e = 0xFF0000;
    ...
    //if else statements which find which byte b is aksing for
    //comparing with & and a full of 1s byte always replicates the desired input bits
    //input is then shifted to the first byte, first 8 bits
    if(b == 0){
        input = input & q;
    }
    ...
    //setting the unsigned c to the uint32 input
    //unsiged c only holds 8 bits, which is what is desired for this function
    c = input;
    return c;
}
...
uint32_t SetByte( uint32_t input, uint8_t value, int b )
{
   ...
   uint32_t g = value;
    ...
    //find the byte
    //set the byte to inputted value
    //zero byte out
    //then ^ value onto it
    if(b == 0){
