...
int main()
{
  ...
  */
  if (Contains(empty, 99) ) {
    ...
    if (IsSorted(v1)){ //this test to see if v1 is sorted. if it is, it will call an error.
        ErrorExit("Sorted() -test 1");
    }
    
    if (!IsSorted(v2)){ //this test to see if v2 is not sorted. if it is notsorted, it will call an error.
        ErrorExit("Sorted() -test 2");
    }

// find min
    std::cout << "Minimum number: " << FindMin(v1) << "\n";

// find max
    std::cout << "Maximum number: " << FindMax(v1) << "\n";
    
// find avg
    std::cout << "Average number: " << Average(v1) << "\n";
